...
<body>
    ...
    <p style="text-align: center; font-size: 0.9em;">Appendix 71 - PAR</p>
    ...
    <table>
        ...
        <tbody>
            <tr>
                ...
                <td>{{ $dateAcquired ?? '—' }}</td>
                <td>{{ $issuance->amount ?? $issuance->unit_cost ?? '—' }}</td>
            </tr>
        </tbody>
    </table>

    <p><strong>Received by:</strong> _______________________ &nbsp; <strong>Issued by:</strong> _______________________</p>
    <div class="signature-block">
        <div>
            <div class="name">{{ $issuance->issuedTo?->name ?? $issuance->custodian_printed_name ?? '_____________________________' }}</div>
            <div class="label">Signature over Printed Name of End User</div>
        </div>
        <div>
